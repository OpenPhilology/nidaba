...
from iris import celery
from iris import storage
from iris.irisexceptions import IrisInputException, IrisNoSuchAlgorithmException, IrisTickException
...
class Batch(object):
    ...
    def add_document(self, doc):
        ...
        Raises:
            IrisInputException: The document tuple does not refer to a file.
        """

        if not storage.is_file(*doc):
            raise IrisInputException('Input document is not a file.')
    ...
    def add_task(self, method, **kwargs):
        ...
        Raises:
            IrisTickException: There is no tick to add a task to.
            IrisNoSuchAlgorithmException: Invalid method given.
        """
        if self.cur_tick == None:
            raise IrisTickException('No tick to add task to.')
        if u'iris.' + method not in celery.app.tasks:
            raise IrisNoSuchAlgorithmException('No such task in registry')
    ...
    def add_tick(self):
        ...
        Raises:
            IrisStepException: There is no step to add a tick to.
        """
        if self.cur_step == None:
            raise IrisStepException('No step to add tick to.')
        if self.cur_tick:
    ...
    def run(self):
        ...
        if len(self.batch_def) > 1:
            ...
            for tset in self.batch_def[1:]:
                for sequence in product(*tset):
                    method = celery.app.tasks['iris.' + sequence[0]['method']]
                    ch = chain(method.s(**(sequence[0])))
                    for seq in sequence[1:]:
                        method = celery.app.tasks['iris.' + seq['method']]
                        ch |= method.s(**seq)
                    tick.append(ch)
        ...
        for doc in self.docs:
            ...
            for sequence in product([doc], *self.batch_def[0]):
                method = celery.app.tasks['iris.' + sequence[1]['method']]
