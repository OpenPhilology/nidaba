...
@app.task(base=IrisTask, name=u'iris.binarize.otsu')
def otsu(doc, id, method=u'binarize', thresh=100, mincount=50, bgval=255,
    ...
    Raises:
        IrisInvalidParameterException: Input parameters are outside the valid
        range.
    ...
    if smoothx < 0 or smoothy < 0 or bgval < 0 or thresh < 0 or mincount < 0:
        raise IrisInvalidParameterException('Parameters (' + 
                                            unicode(thresh) + ',' +
                                            unicode(mincount) + ',' +
                                            unicode(bgval) + ',' +
                                            unicode(smoothx) + ',' + 
...
@app.task(base=IrisTask, name=u'iris.binarize.sauvola')
def sauvola(doc, id, method=u'sauvola', whsize=10, factor=0.35):
    ...
    if whsize < 2 or factor >= 1.0 or factor < 0:
        raise IrisInvalidParameterException('Parameters (' + unicode(whsize)
